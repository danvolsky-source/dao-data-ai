#!/usr/bin/env python3
"""
Optimism DAO Data Collector
Collects proposal and vote data from Optimism Snapshot space
"""
...
def fetch_proposals(space: str = OPTIMISM_SPACE, limit: int = 1000, skip: int = 0) -> List[Dict]:
    """
...
def collect_all_proposals() -> int:
    """
    Collect all proposals from Optimism Snapshot space
    """
    print(f"Starting collection for {OPTIMISM_SPACE}...")
    ...
    while True:
        proposals = fetch_proposals(space=OPTIMISM_SPACE, limit=batch_size, skip=skip)
...
def collect_all_votes() -> int:
    ...
    try:
        result = supabase.table("proposals").select("proposal_id").eq("dao", "optimism").execute()
...
def main():
    ...
    """
    print("=" * 60)
    print("Optimism DAO Snapshot Collector")
    print("=" * 60)
